...
const StudentSignIn = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data) {
        const { isStudent, isTutor } = response.data;
        ...
        if (isStudent) {
          ...
          // If somehow a tutor logged in from student sign-in, redirect to tutor dashboard
          toast.success("Welcome back!");
          navigate("/tutor-dashboard");
        } else {
          toast.error("Invalid account type. Please use the main sign-in page.");
        }
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl text-center">Welcome Back, Student</CardTitle>
          ...
          <CardContent>
            ...
            <div className="mt-6">
              ...
              <div className="text-center text-sm text-muted-foreground">
                ...
                <Link to="/marketplace" className="text-accent hover:underline font-medium">
                  Find a tutor
                </Link>
                {" "}or{" "}
                <Link to="/tutor-signin" className="text-accent hover:underline font-medium">
                  sign in as tutor
                </Link>
              </div>
            </div>
